...
class GUI(tk.Tk):#how to structure this properly?
    ...
    def __init__(self):
        "make gui"
        super().__init__()
        frame1 = tk.Frame(master=self,bg='sienna', width=1000, height=1500, padx= 30, pady=30)
        frame2 = tk.Frame(master=self,bg='sienna', height=500, padx= 30, pady= 30)
        frame3 = tk.Frame(master=self,bg='sienna', height=500, padx= 30, pady=5)
        frame4 = tk.Frame(master=self,bg='sienna', height=1000, padx= 30, pady=10)
        frame5 = tk.Frame(master=self,bg='sienna', height=500, padx= 30, pady=10)
        frame6 = tk.Frame(master=self,bg='sienna',padx=100)
        frame7 = tk.Frame(master=self,bg='sienna', padx= 30, pady=10)
        frame01 = tk.Frame(master=self,bg='sienna',padx= 100, pady=30)
    ...
    def set_data(self):
        ...
        self.filep= os.path.join(self.filepath,'average'+self.timee)    
    ...
    def process_queue(self):#https://stackoverflow.com/questions/16745507/tkinter-how-to-use-threads-to-preventing-main-event-loop-from-freezing
        try:                #https://stackoverflow.com/questions/15323574/how-to-connect-a-progress-bar-to-a-function/15323917#15323917
    ...
    def plot_stuff(self):
        filep=os.path.join(self.filepath,'average'+self.timee)
        mat=np.array(np.loadtxt(filep))
        fig, ax=plt.subplots()
        plt.ylabel('X')
...
class threadedTask(threading.Thread):#Is it better to inherit from 2 classes?
    ...
    def sim_inheritance(self, volt, noa, meas_values, time_con, filepath, sample_name, timee):
        print('finished')
        self.volt=volt
        self.noa=noa
        self.meas_values=meas_values
    ...
    def reading_values(self):
        ...
        for j in range(self.noa):
            ...
            with open((os.path.join(self.filepath,str(j))), 'w+') as fp:
