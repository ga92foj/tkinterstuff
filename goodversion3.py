...
class Main:
    def __init__(self, master):
        ...
        self.average_X = np.zeros([volt.size, noa])
        self.average_Y = np.zeros([volt.size, noa])
        self.average_phase = np.zeros([volt.size, noa])
...
def start():
    a = Main(window)
    if (len(a.loop_start) != 0 and len(a.loop_end) != 0):
        a.loop_start=float(a.loop_start)
        a.loop_end=float(a.loop_end)
        mag_f=get_mag_array_l2r(a.nos, a.loop_start,a.loop_end, a.minfield, a.maxfield)
        mag_b=get_mag_array_r2l(a.nos, a.loop_start,a.loop_end, a.minfield, a.maxfield)
        a.volt=np.concatenate((mag_f,mag_b),axis=None)/1600
    else:
        minvolt=a.minfield/1600
        maxvolt=a.maxfield/1600
        volt1=np.linspace(minvolt,a.maxvolt,a.nos)
        volt2=np.linspace(maxvolt,a.minvolt,a.nos)
        a.volt=np.concatenate((volt1,volt2),axis=None)

    if a.kerrsup == "Rotation":
        sr.harmonic(2)
    elif a.kerref == "Ellipticity":
        sr.harmonic(1)
    else:
        print('error')

    sr.aux_out1(a.volt[0])
    while abs(sr.aux_out1()-a.volt[0])>0.001:
        time.sleep(0.02)
def main_function():
    force_stop=False
    "read data"
